...
import { Button, Drawer, Form, Input, Space, Switch, DatePicker, Typography, Spin } from "antd";
...
export default function PersonDrawerForm({ open, busy, editing, onClose, onCreate, onUpdate }: Props) {
    ...
    const submit = async () => {
        ...
        try {
            ...
            if (!editing) {
                ...
                onUpdate(editing.personId, { ...payloadBase, isActive: !!v.isActive });
            }
        } catch {
            // AntD ya pinta los errores por campo
    ...
    return (
            ...
            footer={
                <Space style={{ justifyContent: "space-between", width: "100%" }}>
                    <Typography.Text type="secondary">
                        {isEdit ? "Update existing record" : "Create a new record"}
                    </Typography.Text>
        ...
        >
            {/* ✅ Loader dentro del formulario mientras guarda/actualiza */}
            <Spin spinning={busy} tip={isEdit ? "Updating person..." : "Creating person..."}>
                <Form form={form} layout="vertical" requiredMark="optional" initialValues={initialValues}>
                    ...
                    <Space style={{ width: "100%" }} size={12}>
                        ...
                        <Form.Item label="Birth date" name="birthDate" style={{ flex: 1 }}>
